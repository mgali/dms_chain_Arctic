...
for iy = years
    ...
    if iy == 2016
        ndperiod = 1 + ndays*(0:34); % 2016 only
    end
    ...
    for ip = ndperiod
        ...
        for iv = 1:length(varnameS)
            ...
            nd = ndays-1;
            if length(ndperiod)>1 && (ip > (365-ndays))
                nd = 365-ip;
                if ~mod(iy,4)
                    nd = nd+1; % leap year
                end
                ...
                if ~status && ~status_ice
                    ...
                    if ~isempty(var_test) % test variable
                        ...
                        if strcmp('Ice',varname)
                            ...
                            Ice1(Ice1==max(Ice1)) = 0; % assign marine pixels not covered by SSMI to Ice=0
                            ...
                            if ~strcmp('Nsst',varname) || ~strcmp('sst_avhrr_4ql',varname)
                                var_grid1(var_grid1<=0) = nan; % remove negative (and zero?) chl values produced by gsm algos
                            end
                            if strcmp('chl_gsm',varname) || strcmp('chl_oc',varname) || strcmp('chl_cota',varname) || strcmp('chl_gsm_mustapha',varname)
                                var_grid1(var_grid1>60) = 60; % Cap at 60 mg m-3 potential extreme Chl values (not either used by dmspt algo)
                            end
                        end
                        ...
                        % Prepare marine pixel count for binning stats
            ...
            if strcmp(var4stats,varname)
                ...
                dlmwrite(sprintf('summary_%s_%i%c_%cgrid_%s.txt',varname,ndays,period,sensor,today),M1,'-append')
                dlmwrite(sprintf('summary65N_%s_%i%c_%cgrid_%s.txt',varname,ndays,period,sensor,today),M1_65,'-append')
                dlmwrite(sprintf('summary_%s_%i%c_%skm_%s.txt',varname,ndays,period,kmgrid2,today),M2,'-append')
                dlmwrite(sprintf('summary65N_%s_%i%c_%skm_%s.txt',varname,ndays,period,kmgrid2,today),M2_65,'-append')
            end
        ...
        if ~isempty(VARSOUT)
            if strcmp(outformat,'netcdf')
                outname = sprintf('%s/%c%c_%i%c_%skm/%0.0f/%c%c%0.0f%03.0f_%i%c_KK.nc',outpath,sensor,sensorATM,ndays,period,kmgrid2,iy,sensor,sensorATM,iy,ip,ndays,period);
                for iv = 1:length(varnameS)
            ...
            elseif strcmp(outformat,'text')
                outname = sprintf('%s/%c%c_%i%c_%skm/%0.0f/%c%c%0.0f%03.0f_%i%c.txt',outpath,sensor,sensorATM,ndays,period,kmgrid2,iy,sensor,sensorATM,iy,ip,ndays,period);
